...
@implementation TOEval {	const char *_code;	BOOL  _abort;	NSUInteger  _depth,  _size;	}
...
- (void) abort																		{	if (_depth) _abort = YES;	} /* OK */
+ (id) evalStatement: (NSArray*)statement									{

	return [(TOEval*)[self.alloc initWithStatement:statement mem:TOMem.new] eval];
} /* OK */
...
- (id) evalStatement: (NSArray*)statement									{
	...
	++_depth > _size	?	^{
		...
		_abort = YES;
	}() :	(_abort) ? [self logAt:index line:@"aborted"]
		 : ^{
...
- (id) evalAssignment:(NSArray*)statement index:(NSUInteger)index	{
	...
	if ([target isKindOfClass:NSArray.class]) {
		if ([target count] == 4 && [target[0] characterAtIndex:0] == TOTypeMethod) {
			...
			NSString *s = [NSString stringWithFormat:@"set%@%@:", [target[3] substringToIndex:1].uppercaseString, [target[3] substringFromIndex:1]];
...
- (id) evalValue:		 (NSArray*)statement index:(NSUInteger)index	{
	...
	switch (type) {
		case 'a': {
			result = NSMutableArray.new;// [[NSMutableArray alloc] initWithCapacity:[value count]];
			for (id s in value) {
		...
		case 'd': {
			result = NSMutableArray.new;//[[NSMutableDictionary alloc] initWithCapacity:[value count]];
...
- (id) performOnTarget:(id)target selectorString:(NSString*)selector 
				 arguments:(NSArray*)arguments index:(NSUInteger)index	{
	...
	if (target) {
		...
		} else	[self logAt:index line:@"Class %@ does not respond to '%@'", [target class], selector];
	}
	return nil;
} /* OK */
#pragma mark - Logging
- (void)logAt:(NSUInteger)index line:(NSString*)format, ...			{
	...
	NSString *expect = @"";
	if (format.length) {		va_list args;		va_start(args, format);
		expect = [[NSString alloc] initWithFormat:format arguments:args];
		va_end(args);
	}
	if (_source)		[_delegate log:[TOMem formatAt:index code:_source.UTF8String string:expect]];
	else if (expect) 	[_delegate log:[NSString stringWithFormat:@"%@ at %i",  expect, (int)index]];
} /* OK */
@end
