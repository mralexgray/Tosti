...
-(NSAttributedString*)tokenForString:(NSString*)aString	{
   ...
   [as addAttribute:NSAttachmentAttributeName value:ta range:NSMakeRange(0, [as length])];
	[as addAttribute:NSBaselineOffsetAttributeName value:@0 range:NSMakeRange(0, [as length])];
	return	as;
}
-(NSArray*)getCompletionsForStem:(NSString*)stem			{
   MTTokenField * controlView = (MTTokenField*)[self delegate];
	if (![controlView respondsToSelector:@selector(delegate)]) return NSArray.new;
...
-(NSRange)forwardCompletionRange									{
	...
	if (effectiveRange.location<[[self textStorage]length]){
		...
		if (lastRange.location == NSNotFound)			lastRange.location=0;
	}
	return lastRange;
}
...
-(NSRange)completionRange											{
	...
	if (effectiveRange.location == NSNotFound)	effectiveRange.location=0;
...
-(NSString*)completionStem											{
	...
	if (completionRange.location+completionRange.length-1>strLen)		return @"";
	return [self.textStorage.string substringWithRange:completionRange];
}
...
-(void) insertTokenForText:(NSString*)tokenText replacementRange:(NSRange)replacementRange										{
	if ([(MTTokenField*)[self delegate] shouldAddToken: tokenText atTokenIndex:
								[self countOfTokensInRange:NSMakeRange(0,replacementRange.location)]]) {
...
-(void)insertText:(id)aString replacementRange:(NSRange)replacementRange 																{
	[[_MTTokenCompletionWindowController sharedController] isDisplayingCompletions] ?
	[super insertText:aString replacementRange:replacementRange] :
	...
	if (insertionIndex !=NSNotFound){
		if ([[self textStorage] length]==insertionIndex){
			...
			if ([aString length]==1 && [[(MTTokenField*)[self delegate] tokenizingCharacterSet] characterIsMember:[aString characterAtIndex:0]]){
				...
				else	// just insert the text without completing -- don't call super insertText as that will start a completion by default.
...
-(void)commitTokens								{	[self tokenizeRemainingText];	}
-(BOOL)resignFirstResponder					{	[self tokenizeRemainingText];	return [super resignFirstResponder];	}
