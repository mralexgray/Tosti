...
-(id)initWithTitle:(NSString*)aTitle {
	...
	title = [aTitle copy];
	_MTTokenTextAttachmentCell* tac = _MTTokenTextAttachmentCell.new;
	tac.tokenTitle = title;
	[self setAttachmentCell: tac];
	[tac release];
	return self;
}
-(id)description{	return [NSString stringWithFormat:@"<%@ %p Title: %@ | cell: %@>",[self class]  ,self, self.title,[self attachmentCell]];}
-(id)copyWithZone:(id)zone{	return nil; }
-(id)mutableCopy{	return nil;   }
-(void)dealloc{
